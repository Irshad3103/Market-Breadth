        ...
        {chartData.map((data, index) => (
          <div key={data.period} className="relative">
            <div className="flex justify-between items-center mb-2">
              ...
              <span className={`text-sm font-bold ${data.percentage >= 70 ? 'text-green-400' :
            ...
            <div className="w-full bg-white/10 rounded-full h-4 relative overflow-hidden">
              <div
                className={`h-4 rounded-full transition-all duration-1000 ease-out ${data.percentage >= 70 ? 'bg-gradient-to-r from-green-500 to-green-400' :
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <span className={`font-semibold ${
            marketBreadth.marketHealth === 'bullish' ? 'text-green-400' :
              marketBreadth.marketHealth === 'very strong' ? 'text-green-300' :
                marketBreadth.marketHealth === 'strong' ? 'text-emerald-400' :
                  marketBreadth.marketHealth === 'neutral' ? 'text-yellow-400' :
                    marketBreadth.marketHealth === 'weak' ? 'text-orange-400' :
                      marketBreadth.marketHealth === 'bearish' ? 'text-red-400' : 'text-yellow-400'
            }`}>
            {marketBreadth.marketHealth.toUpperCase().replace('VERY STRONG', 'VERY STRONG')}
          </span>
        </div>
      </div>
    </div>
